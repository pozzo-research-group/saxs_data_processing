...
def mcsas_fit(fp_load, fp_write, n_threads=40):
    ...
    mds = McData1D.McData1D(
        ...
        csvargs={
            "sep": "\s+|\t+|\s+\t+|\t+\s+",  # field delimiter, for flexible whitespace, use: "\s+|\t+|\s+\t+|\t+\s+" (https://stackoverflow.com/questions/15026698/how-to-make-separator-in-pandas-read-csv-more-flexible-wrt-whitespace-for-irreg#15026839)
            ...
            "index_col": False,
        },  # no index column before every row (who does this anyway?)
    )
    ...
    mh = McHat.McHat(
        ...
        fitParameterLimits={
            "radius": (20, 250)
        },  # this is the parameter we want to MC optimize within these bounds
